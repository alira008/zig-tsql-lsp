...
pub const Tag = enum {
    ...
    pub fn lookupKeyword(ident: []const u8, sql_dialect: dialect.Dialect) ?Tag {
        var buf = [_]u8{0} ** 20;
    ...
    pub const core_keywords = std.StaticStringMap(Tag).initComptime(.{
        .{ "with", .kw_with },
        .{ "select", .kw_select },
    ...
    pub const postgres_keywords = std.StaticStringMap(Tag).initComptime(.{
    ...
    pub const sqlserver_keywords = std.StaticStringMap(Tag).initComptime(.{
        .{ "exec", .kw_exec },
        .{ "top", .kw_top },
        .{ "declare", .kw_declare },
        .{ "set", .kw_set },
